...
def particlefilter_torch(G, J, U, V, lam, r, y, Q_process, Q_obs, Np):
    ...
    for tt in range(T):
        ...
        WVec = WVec*(torch.exp(-0.5*(rMinvr.squeeze(2) + torch.sum(f_tap*Pinvf_tap - v*mu_post,dim=1))) + epsilon)
...
def Qfunction_torch(G, J, U, V, lam, r, y, Particles, Weights, Q_process, Q_obs):

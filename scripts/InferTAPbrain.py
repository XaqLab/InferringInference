...
def main():
    ...
    q_obs = float(sys.argv[10])  # variance of independent measurement noise to add
    ...
    # Ns += 10  ## USE THIS OFFSET ONLY WHEN NECESSARY
    print('assumed no. of latent variables =', Ns)
    ...
    T_clip = 20
    r_ICA = generate_TAPbrain_dynamics(
        tapbrain,
        theta,
        params,
        B_ICA,
    ...
    U_init = UhatICA(
        np.reshape(r_ICA.transpose(1, 2, 0) - b_init, [Nr, B_ICA * T_ICA], order='F').T,
        Ns,
    )[0]
    del r_ICA

    # initialize the rest of the parameters
    G_init = 0.1 * np.random.randn(18)
    G_init[0] = 0
    G_init[9] = 0
    ...
    T = 25
    yG_low, yG_high = 5, 25
    y, x, r_brain = generate_TAPbrain_dynamics(
        tapbrain, theta, params, B, T, T_low, T_high, yG_low, yG_high, T_clip, use_cuda
    )
    ...
    J_hat_vec = torch.tensor(
        JMatToVec(J_init), device=device, dtype=dtype, requires_grad=True
    )
    J_hat = JVecToMat_torch(J_hat_vec, Ns)
    b_hat = (
        torch.tensor(b_init, device=device, dtype=dtype, requires_grad=True)
        if model_type
        else 0
    )
    ...
    B_EVAL = 200
    B_eval = B_EVAL if B > B_EVAL else B

    with torch.no_grad():
        LL_tp = particlefilter(
            G,
            J,
            U,
            V,
            lam,
            r_brain[0:B_eval] - baseline,
            y[0:B_eval],
            P_pr_true,
            P_obs,
            Np,
    ...
    with torch.no_grad():
        LL_init = particlefilter(
            G_hat,
            J_hat,
            U_hat,
            V_hat,
            lam,
            r_brain[0:B_eval] - b_init,
            y[0:B_eval],
            P_process,
            P_obs,
            Np,
    ...
    print(
        'log likelihood with initial parameters = %.1f' % (LL_init.mean().data.numpy())
    )
    ...
    for iteration in range(NIterations):
        ...
        if iteration == NIterations // 2:
            optimizer = torch.optim.Adam(
                opt_params, lr=learning_rate / 4, betas=(0.9, 0.999)
            )

        if iteration == 3 * NIterations // 4:
            optimizer = torch.optim.Adam(
                opt_params, lr=learning_rate / 16, betas=(0.9, 0.999)
            )
        ...
        idx = np.random.randint(low=0, high=B - B_eval, size=batch_size)

        # run particle filter to get posterior for E-step
        with torch.no_grad():
            LL_b, x_b, P_b, W_b = particlefilter(
                G_hat,
                JVecToMat_torch(J_hat_vec, Ns),
                U_hat,
                V_hat,
                lam,
                r_brain[idx] - b_hat,
        ...
        # M-step
        C.backward()
        G_hat.grad[0], G_hat.grad[9] = (
            0,
            0,
        )  # set gradient of G0 and G9 to zero:  coefficient of x^0 terms
        G_hat.grad[8], G_hat.grad[17] = (
            0,
            0,
        )  # set gradient of G0 and G9 to zero: coefficients of J^2 x^4 terms
    ...
    J_hat = JVecToMat_torch(J_hat_vec, Ns)
    with torch.no_grad():
        LL_hat = particlefilter(
            G_hat,
            J_hat,
            U_hat,
            V_hat,
            lam,
            r_brain[0:B_eval] - b_hat,
            y[0:B_eval],
            P_process,
            P_obs,
            Np,
        )[0]
        LL_hat_test = particlefilter(
            G_hat,
            J_hat,
            U_hat,
            V_hat,
            lam,
            r_brain[B - B_eval :] - b_hat,
            y[B - B_eval :],
            P_process,
            P_obs,
            Np,
    ...
    print(
        'log likelihood with inferred parameters = %.1f' % (LL_hat.mean().data.numpy())
    )
    print(
        'log likelihood with inferred parameters on test data = %.1f'
        % (LL_hat_test.mean().data.numpy())
    )
    ...
    inferred_parameters = {
        'G_hat': G_hat.data.numpy(),
        'J_hat': J_hat.data.numpy(),
        'U_hat': U_hat.data.numpy(),
        'V_hat': V_hat.data.numpy(),
        'b_hat': b_hat,
        'LL_hat': LL_hat,
        'LL_hat_test': LL_hat_test,
    }
